...
#        plt.show()
    for k in range(3,6) :
        ...
        for j in range(4):
            ...
            plt.bar(abcisse + i * 0.32 + j * 0.06, abs(res_em[j, :, k]), width = L, color = color_list[j], hatch = hatch_list[i])
        plt.ylabel(tag_list[k])
        plt.title('Input SNR = ' + str(iSNR) + ' dB')
        plt.xticks(abcisse + 0.4, bars, rotation=45, fontweight='bold')
...
for k in range(6) :
    plt.figure(k)
    for i in range(len(p_list)) :
        plt.bar(abcisse + i * 0.32 + 4 * 0.06, abs(res_amp[i, :, k]), width = L, color = 'c', hatch = hatch_list[i])
